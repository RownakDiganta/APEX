...
async def test_health_does_not_expose_env_or_internal_paths() -> None:
    ...
    async with client_for(app) as client:
        r = await client.get("/health")
    body = r.json()
    # Phase 22 "bounded_file_read", §28.16 "ftp_validate", §28.17 "ftp_read" are
    # static capability flags only (the endpoints exist); none reads a file,
    # validates a path/credential, or exposes paths/basenames/executables/targets.
    assert set(body.keys()) == {
        "status", "service", "tools", "bounded_file_read", "ftp_validate", "ftp_read",
    }
    assert isinstance(body["bounded_file_read"], bool)
    assert isinstance(body["ftp_validate"], bool)
    assert isinstance(body["ftp_read"], bool)
