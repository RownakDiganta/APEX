...
Then start the safe, target-free connectivity check (builds both images,
waits for `kali` to report healthy, then runs a bounded dry-run smoke
check that exits cleanly):
...
- **Default Compose behavior is only a smoke test.** `apex`'s default
  command is a bounded, dry-run connectivity check — it never contacts
  `kali` for anything but a health check, and never begins an engagement.
- **`APEX_DRY_RUN=true` is the safe default** — set in `.env.example`,
  matching `ApexConfig.dry_run`'s own hardcoded default. An environment
  variable can never flip this to `false` by itself; real execution always
  requires the explicit `--no-dry-run` CLI flag on `apex_host.main` /
  `apex_host.eval.run_htb_local` (CLAUDE.md §13.5).
...
To actually exercise the real `apex`-to-`kali` HTTP call (still no real
target — `curl --version` only), pass `--no-dry-run` explicitly to the
same smoke module:
