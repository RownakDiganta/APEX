...
class LexicalIndex(Protocol):
    """BM25-based full-text index.

    Entries carry a ``metadata`` dict that MUST include a ``"tier"`` key so
    that the retrieval engine can filter results.
    """
...
class VectorIndex(Protocol):
    """Dense approximate-nearest-neighbour index."""
