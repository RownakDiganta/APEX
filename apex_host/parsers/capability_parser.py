...
Three derivation methods are implemented: ``derive_ssh_capability``
(Phase 18 / access-capability refactor), ``derive_direct_file_read_capability``
...
value passed in), and ``derive_command_capability`` (Phase 21 — covers
``local_shell``, ``remote_command``, and ``web_command``; the first two
share ``BoundedCommandCapabilityAdapter`` at the runtime-registration
layer, while ``web_command`` shares ``DirectFileReadCapabilityAdapter`` —
see ``apex_host.orchestration.dispatch_node._register_capability_adapter``
— but all three share THIS ONE derivation method, since the *evidence*
required to prove "APEX can execute one bounded, read-only command through
this channel" is the same regardless of which adapter ultimately services
it). Adding a further capability type means adding one more ``derive_*``
method here (and one more registration branch in
...
#: The only capability types `derive_command_capability` may ever produce.
#: ``local_shell`` and ``remote_command`` are serviced by
#: ``BoundedCommandCapabilityAdapter``; ``web_command`` is serviced by
#: ``DirectFileReadCapabilityAdapter`` (a deliberate adapter-sharing
#: decision — see module docstring) but its EVIDENCE requirements are
#: command-execution-shaped, not file-serving-shaped, so it is derived
#: through this method rather than `derive_direct_file_read_capability`.
_COMMAND_CAPABILITY_TYPES: frozenset[AccessCapabilityType] = frozenset({
    AccessCapabilityType.local_shell, AccessCapabilityType.remote_command,
    AccessCapabilityType.web_command,
})

#: The ONLY validation methods `derive_command_capability` accepts as
#: "positive evidence demonstrating a controlled, bounded, read-only
#: command execution." An HTTP 200, an LLM's own assertion that "command
#: injection succeeded," output merely containing a common OS word, a
#: shell-like error message, discovered credentials, or application
#: administrator access are all deliberately NOT in this set — none of
#: those demonstrate that APEX itself can invoke a specific, bounded,
#: read-only command and observe its real output.
_ACCEPTED_COMMAND_VALIDATION_METHODS: frozenset[str] = frozenset({
    # An operator has manually, out-of-band confirmed (through authorized
    # testing) that a specific, fixed command shape executes and returns
    # readable output through this channel — the same trust boundary
    # already established for --username/--password and for
    # direct-file-read's own "operator_attestation".
    "operator_attestation",
    # A harmless, fixed, operator-approved canary command was executed and
    # its output matched exactly what was expected (proves controlled
    # execution, not merely "some response was produced").
    "canary_output_match",
    # A single-use random value was embedded in the command/expected output
    # and the observed result contained that exact nonce — proves the
    # response was generated by a real execution of THIS request, not a
    # cached/static/coincidental response.
    "nonce_bound_execution",
    # A fixed, deterministic, universally benign command (e.g. one whose
    # output is predictable and content-verifiable) was executed and its
    # output was verified against the expected deterministic result.
    "deterministic_benign_command",
    # An existing, already-authorized execution backend confirmed (via its
    # own session/handle validation) that this specific session can run a
    # bounded read action — the backend's own confirmation is the evidence,
    # not a fresh probe performed here.
    "backend_confirmed_session",
})

#: Below this confidence, `derive_command_capability` refuses to derive a
#: capability regardless of `validation_method`.
_MIN_COMMAND_CAPABILITY_CONFIDENCE = 0.6

#: Confidence assigned to a freshly-derived command capability when the
#: caller does not supply an explicit, evidence-appropriate value.
_DEFAULT_COMMAND_CAPABILITY_CONFIDENCE = 0.7
...
class CapabilityParser:
    ...
    def derive_command_capability(
        ...
        *,
        ...
        source_task_id: str,
        validation_method: str,
        confidence: float = _DEFAULT_COMMAND_CAPABILITY_CONFIDENCE,
        source_node_id: str = "",
        max_output_bytes: int = 4096,
        strategy_id: str = "",
    ) -> ParsedObservation:
        """Build the ``access_capability`` node + edges for a validated
        bounded command-execution primitive (Phase 21 — ``local_shell``,
        ``remote_command``, or ``web_command``).

        This is a GENERIC capability derivation, not a specific-exploit
        derivation — it never encodes how the underlying execution primitive
        was discovered (an authenticated web command endpoint, a validated
        command-injection primitive, an established web-shell-like channel,
        a validated local shell/session handle, a validated remote command
        channel, an existing authorized execution backend, ...). Only the
        fact that APEX can now perform one bounded, read-only command
        execution through it is recorded.

        Requires structured, positive evidence — refuses to derive a
        capability when:

        - ``validation_method`` is not one of the recognised
          ``_ACCEPTED_COMMAND_VALIDATION_METHODS`` (an HTTP 200 alone, an
          LLM's own claim of successful command injection, output merely
          containing a common OS word, a shell-like error, discovered
          credentials, or application administrator access are all NOT
          acceptable evidence — see module docstring);
        - ``confidence`` is below ``_MIN_COMMAND_CAPABILITY_CONFIDENCE``;
        - ``capability_type`` is not one of ``local_shell`` /
          ``remote_command`` / ``web_command``;
        - ``principal`` is empty.

        On acceptance, the node's ``metadata`` prop records
        ``validation_method``/``max_output_bytes``/``strategy_id`` (a
        sanitized, opaque label identifying which fixed strategy binding
        produced this capability — never a command string, shell payload,
        session token, or raw canary/flag value) plus ``read_only: True``
        (this capability supports read-only objective verification only —
        never a generic write/execute action). Edges: the same
        ``host --has_capability--> access_capability`` (always) and
        optional ``source_node_id --enables--> access_capability`` shape
        every other ``derive_*`` method here uses.

        ``runtime_available`` starts ``False`` — no runtime adapter/strategy
        is registered until the orchestration layer successfully constructs
        one (``apex_host.orchestration.dispatch_node
        ._register_bounded_command_adapter`` for ``local_shell``/
        ``remote_command``, or ``._register_direct_file_read_adapter`` for
        ``web_command``).
        """
        if validation_method not in _ACCEPTED_COMMAND_VALIDATION_METHODS:
            return ParsedObservation()
        if confidence < _MIN_COMMAND_CAPABILITY_CONFIDENCE:
            return ParsedObservation()
        if capability_type not in _COMMAND_CAPABILITY_TYPES:
            return ParsedObservation()
        if not principal:
            return ParsedObservation()
