...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "the exact model id with your provider account; e.g. a native OpenAI "
            "model id for provider=openai, a native Anthropic model id for "
            "provider=anthropic, or a vendor/model route for provider=openrouter."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "endpoint (applies only to whichever --llm-provider is chosen; "
            "never carried across providers). Prefer --llm-openai-base-url / "
            "--llm-anthropic-base-url / --llm-openrouter-base-url for new "
            "configuration. Leave unset to use each provider's own official "
            "default endpoint."
        ),
    )
    parser.add_argument(
